...
Логіка:
  1. Отримати з БД квоти для заданої категорії (скільки питань з кожного розділу).
...
def generate_test(
    ...
    # Отримуємо квоти для цієї категорії
    quotas = db.get_quotas_for_category(cat_id, db_path)
    if not quotas:
        raise ValueError(f"Квоти для категорії '{category_code}' не знайдені. "
